...
polymer_lengths = [int(x) for x in lines[0].replace(" ", "").split("=")[1].strip("[]").split(",")]  # Converting first line into a list of integers
monomer_length, edge_diameter, dim, angle_interval, mean_err, max_tries = [float(value.replace(" ", "").split("=")[1]) for value in lines[1:]]  # Assigning all the other variables
...
# Main program starts here, for every length of polymer given
for polymer_length in polymer_lengths:
    ...
    while not valid:  # Repeat until the condition is met
        ...
        if condition < mean_err or len(radii) > max_tries:  # We could also stop if there have been more than max_tries Radii calculated
            valid = True
